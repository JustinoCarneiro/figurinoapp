...
3. `DESIGN.md` — conceito visual (Direção B · Teatral Clássico) + regras de aplicação + do/don't.
...
**Bordô é pontual.** Use como cor de **ação** (CTAs, links, ícones interativos). Nunca como fundo de seção grande. A maior parte da tela é creme.

**Estados da locação têm cor codificada** (ver `.badge-*` em `tokens.css`):
- Disponível → verde-musgo · Em uso → bordô · Aguardando pagamento → âmbar · Atrasada → vermelho-tijolo · Em manutenção → dourado · Finalizada → cinza-creme.

## 4. As 12 telas a gerar (em ordem)

Ordem proposital: começamos pelo **fluxo crítico** (figurinista cria locação → admin confirma → devolução), depois cadastros, por último relatórios. Isso permite aprovar o coração do sistema antes de gastar tempo em telas periféricas.

### Bloco 1 — Acesso e Home (fundação)
1. **Login** — único formulário, usuário + senha. Diferenciação por perfil acontece no backend. Identidade do TJA no topo (logo + nome do sistema em Cormorant Garamond).
...
12. **Relatórios — Peças Fora** — atende H7.2. Tabela: peça, locatário, data de saída, data prevista, dias de atraso (se houver). Linhas atrasadas em vermelho-tijolo soft.

## 5. Definição de "tela pronta"

Cada tela só é aprovada quando atende as 3 camadas da definição de pronto do projeto:

- **Belo** — respeita rigorosamente a Direção B do `DESIGN.md`. Tipografia, cores, espaçamentos, raios saem do `tokens.css`.
...
- Não usar Cormorant Garamond em texto de UI (botão, input, label). Apenas em títulos.
- Não somar aluguel + caução num número único.
- Não usar texto cinza-claro sobre fundo creme (figurinista não enxerga).
...
- Não copiar padrões de "SaaS moderno genérico" (sidebar escura, gradientes neon). Esse sistema mora num teatro centenário — a personalidade da Direção B tem que aparecer.
...
*Briefing v1.0 · Fase 2b · Junho/2026.*
