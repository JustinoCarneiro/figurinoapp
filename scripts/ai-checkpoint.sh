...

{
  ...
  if [ "$onda_run_tests" -eq 1 ]; then
    # Ainda na fase de spec/protótipo (sem backend nem suíte). Validação é
    # conferir spec.md/ROADMAP.md e o protótipo em src/ no navegador.
    echo "| (sem suíte automatizada — fase spec/protótipo) | — | conferência manual |"
  else
